...
- [ ] PR의 연결 이슈와 What/Why/How를 확인한다.
- [ ] 변경 사항을 검증한다. 코드 변경은 정상 동작과 경계값·예외 처리를, 문서 변경은 내용의 정확성과 링크를 확인한다.
- [ ] 파일과 라인을 근거로 구체적인 리뷰 의견을 남긴다.
- [ ] 작성자는 피드백 반영 커밋 또는 기술적 근거를 담은 답글을 남긴다.
- [ ] 병합 전에 동료 승인, 리뷰 대화 해결, 충돌 없음 여부를 확인한다.

### 실무 팁

- 리뷰 의견에는 발견한 상황, 영향, 개선 제안을 함께 적는다.
- 피드백 반영 후에는 변경한 커밋과 검증 결과를 답글로 남겨 재확인을 돕는다.
- 문서만 변경한 경우 링크와 내용의 일관성을 검증하고 PR에 검증 방법을 적는다.
